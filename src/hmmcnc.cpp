...
public:
  ...
  int operator<(const SNV &rhs) const { return pos < rhs.pos; }
};

void Reset(vector<vector<double>> &vi) {
  for (std::vector<double> &vj : vi) {
...
public:
  ...
  vector<vector<double>> *transP, *emisP, *expTransP;
...
//
// Not super
void ReadCoverage(string &covFileName, vector<string> &contigNames,
                  vector<vector<int>> &covBins) {
...
void WriteSNVs(string &snvFileName, vector<string> &contigNames,
               vector<vector<SNV>> &snvs) {

  ofstream snvOut(snvFileName.c_str());
  for (int c = 0; c < contigNames.size(); c++) {
    for (int i = 0; i < snvs[c].size(); i++) {
      snvOut << contigNames[c] << "\t" << snvs[c][i].pos << "\t"
             << snvs[c][i].refNuc << "\t" << snvs[c][i].altNuc << "\t"
             << snvs[c][i].ref << "\t" << snvs[c][i].alt << endl;
    }
  }
}

void ReadSNVs(string &snvFileName, vector<string> &contigNames,
              vector<vector<SNV>> &snvs) {
...
void WriteCovBed(string &covFileName, vector<string> &contigNames,
                 vector<vector<int>> &covBins) {
  ofstream covFile(covFileName.c_str());
  for (int c = 0; c < contigNames.size(); c++) {
    for (int i = 0; i < covBins[c].size(); i++) {
      covFile << contigNames[c] << "\t" << i * 100 << "\t" << (i + 1) * 100
              << "\t" << covBins[c][i] << endl;
    }
  }
}
...
double max_over_rows(vector<vector<double>> &v, size_t col,
                     vector<vector<double>> &v2, size_t nextState,
                     size_t nStates) {
...
static void correctModel(vector<vector<double>> &transP, int nStates) {
  double sum;
...
void CombineEmissions(vector<int> &obs, vector<SNV> &snvs,
...
double CSEmisP(int state, int pos, vector<int> &obs, vector<SNV> &snvs,
               vector<uint8_t> &isCov, vector<int> &obsIndex,
  ...
  for (int k = totObs - 1; k > 0; k--) {
    for (int i = 0; i < nCovStates; i++) {
      ...
      for (int j = 0; j < nCovStates; j++) {
        ...
        colSum = PairSumOfLogP(colSum, b[j][k + 1] + covCovTransP[j][i] +
                                           emisP[j][obs[k]]);
      }
      b[i][k] = colSum;
    }
...
double BaumWelchEOnChrom(vector<double> &startP,
  ...
  int nObs = obs.size();
  double px;
  px = ForwardBackwards(startP, covCovTransP, emisP, obs, f, b);
  /*
  ofstream fb("fb.tsv");
  for (int k=0; k < f[0].size()-1; k++) {
    ...
    for (int j=0; j < nStates; j++) {
      double p=f[j][k] + b[j][k+1];
      fb << std::setprecision(8) << f[j][k] << ", " << b[j][k+1] << ", " <<
  f[j][k] + b[j][k+1];
    ...
    fb << "\t" << maxi << "\t" << maxfb/fbSum;
    if (isCov[k]) {
      fb << "\tCOV, " << obs[obsIndex[k]];
    }
    else {
      fb << "\tSNV, " << snvs[obsIndex[k]].ref << ", " << snvs[obsIndex[k]].alt;
    }
    fb << endl;
  }
  fb.close();
  ...
  for (int k = 1; k < nObs - 1; k++) {
    ...
    for (int i = 0; i < covCovTransP.size(); i++) {
      ...
      for (int j = 0; j < covCovTransP[i].size(); j++) {
        logSum = PairSumOfLogP(logSum, f[i][k] + covCovTransP[i][j] +
                                           emisP[j][obs[k + 1]] + b[j][k + 1]);
      }
    }
    for (int i = 0; i < covCovTransP.size(); i++) {
      for (int j = 0; j < covCovTransP[i].size(); j++) {
        double pEdge =
            f[i][k] + covCovTransP[i][j] + emisP[j][obs[k + 1]] + b[j][k + 1];
...
void PrintIntervals(string chrom, vector<Interval> &intv, ostream &out) {
  for (int i = 0; i < intv.size(); i++) {
    out << chrom << "\t" << intv[i].start << "\t" << intv[i].end << "\t"
        << intv[i].copyNumber << "\t" << intv[i].averageCoverage << "\t"
        << intv[i].pVal << endl;
  }
}
void StorePosteriorMaxIntervals(vector<int> &cov, vector<vector<double>> &f,
...
string version = "0.8";
string reference;
void WriteVCF(ostream &out, string &refName, string &sampleName,
              vector<string> &contigNames, vector<int> &contigLengths,
...
void BaumWelchM(vector<double> &startP, vector<vector<double>> &transP,
                vector<vector<double>> &emisP,
                vector<vector<vector<double>>> &binoP, int model,
                vector<long> &stateTotCov, vector<long> &stateNCov,
  ...
  for (int j = 0; j < nStates; j++) {
    ...
    for (int k = 0; k < nStates; k++) {
      updateTransP[j][k] =
          log(expTransP[j][k] / colSum); // min(ONE, expTransP[j][k] - colSum);
  ...
  for (size_t i = 0; i < nStates; i++) {
    updateEmisP[i].resize(maxCov);
    if (stateMean[i] > 0) {
      //      cout << "Updating emissions for " << i << " with mean " <<
      //      stateMean[i] << "\t" << stateNCov[i] << endl;
      for (size_t j = 0; j < maxCov; j++) {
        if (model == POIS) {
          updateEmisP[i][j] = LgPrpoiss(1, j, (int)stateMean[i]);
        } else {
          // For now assume shape has var=2*mean (as observed with cn=2)
          updateEmisP[i][j] =
              LgNegBinom(1, (int)j, stateMean[i], stateMean[i] * 2);
        }
      }
    } else {
      //      cout << "Not updating emissions for " << i << endl;
    }
  }
}

void viterbi(vector<double> &startP, vector<vector<double>> &covCovTransP,
             vector<vector<double>> &covSnvTransP,
             vector<vector<double>> &snvSnvTransP,
             vector<vector<double>> &emisP,
             vector<vector<vector<double>>> &binoP, vector<int> &cov,
             vector<SNV> &snvs, vector<uint8_t> &isCov, vector<int> &obsIndex,
...
public:
  int index;
  int count;
  int operator<(const CountNuc &rhs) { return count < rhs.count; }
};
...
int StoreSNVs(char *contigSeq, int contigLength, float mean, vector<int> &nA,
  ...
  for (int i = 0; i < contigLength; i++) {
    ...
    if (counts[4].index != 4 and counts[3].index != 4 and refNuc != 'N' and
        counts[3].count > 0.25 * mean and counts[4].count > 0.25 * mean) {
      //
  ...
  while (*(threadInfo->lastSeq) < (*(*threadInfo).contigNames).size()) {
    ...
    *(threadInfo->lastSeq) = *(threadInfo->lastSeq) + 1;
    ...
    pChrom = BaumWelchEOnChrom(
        *threadInfo->startP, *threadInfo->transP, *threadInfo->emisP,
        (*threadInfo->covBins)[curSeq], f, b, expCovCovTransP, expEmisP);
...
void ParseChrom(ThreadInfo *threadInfo) {
  ...
  while (*(threadInfo->lastSeq) < (*(*threadInfo).contigNames).size()) {
    ...
    *(threadInfo->lastSeq) = *(threadInfo->lastSeq) + 1;
    ...
    hts_itr_t *regionIter = sam_itr_querys(
        threadInfo->bamidx, threadInfo->samHeader, region.c_str());
    int chromLen;
    ...
    while (continueParsing) {
      ...
      while (bufSize < 100000000 and continueParsing) {
        ...
        if (res < 0) { // or totalReads < 15000) {
          ...
          cerr << "Ending parsing of " << region << " with " << totalSize
               << " data and " << chunkNumber << " iterations on res " << res
               << endl;
          break;
        }
        endpos = bam_endpos(b);
      ...
      cerr << "Reading contig index " << curSeq << " (chunk " << chunkNumber
           << ")\t" << (*threadInfo->contigNames)[curSeq] << " read "
           << reads.size() << " (" << totalReads << ").\tEnding pos: " << endpos
           << endl;
      ++chunkNumber;
      pthread_mutex_unlock(threadInfo->semaphore);
...
int GetRefAndAlt(char refNuc, vector<int> &counts, int &ref, int &alt) {
  ...
  std::sort(sCounts.begin(), sCounts.end());
...
int EstimateCoverage(string &bamFileName, vector<vector<int>> &allCovBins,
                     vector<string> &chroms, vector<int> &lengths,
                     string &useChrom, double &mean, double &var) {
  int useChromIndex = 0;
  if (useChrom == "") {
    int maxLen = 0;
...
void WriteParameterFile(string &fileName, int nStates, double covMean,
                        double covVar, int maxState, int maxCov,
                        vector<double> &startP, vector<vector<double>> &transP,
                        vector<vector<double>> &emisP) {
...
void ReadParameterFile(string &fileName, int &nStates, double &covMean,
                       double &covVar, int &maxState, int &maxCov,
                       vector<double> &startP, vector<vector<double>> &transP,
                       vector<vector<double>> &emisP) {
...
void ReadFai(string faiFileName, vector<string> &contigNames,
             vector<int> &contigLengths) {
...
void InitParams(vector<vector<double>> &covCovTransP,
                ...
                int nSNVStates, double diag, double offDiag,
...
void PrintHelp() {
  cout
      << "usage: hmmcnc reference.fa" << endl
      << "   -a alignments    Read alignments from this file and calculate "
         "depth on the fly."
      << endl
      << "   -b bed           Read depth bed from this file. Skip calculation "
         "of depth."
      << endl
      << "   -s snv-file      Read SNVs from this file (when not estimating "
         "from a BAM)"
      << endl
      << "   -p parameter     Read parameter file (do not train with "
         "Baum-Welch)"
      << endl
      << " Options controlling depth calculation " << endl
      << "   -e value (float)   Value of log-epsilon (-500)." << endl
      << "   -m value [pois|nb] Coverage model to use, Poisson (pois), or "
         "negative binomial (nb). Default nb."
      << endl
      << "   -x value Max state to allow (10)" << endl
      << " -t value (int)     Number of threads (4) " << endl
      << " -c contig          Use this contig to estimate coverage. By "
         "default, longest contig."
      << endl
...
int main(int argc, const char *argv[]) {
  ...
  vector<vector<double>> updateEmisP;
  vector<vector<vector<double>>> binoP;
  vector<vector<double>> expCovCovTransP, expCovSnvTransP, expSnvSnvTransP,
      expEmisP;
  ...
  if (paramInFile != "") {
    ReadParameterFile(paramInFile, nStates, mean, var, maxState, maxCov, startP,
                      covCovTransP, emisP);
  }
  ...
  for (int procIndex = 0; procIndex < nproc; procIndex++) {
    ...
    threadInfo[procIndex].fai =
        fai_load_format(referenceName.c_str(), FAI_FASTA);
    if (nproc > 1)
  ...
  EstimateCoverage(bamFileName, covBins, allContigNames, allContigLengths,
                   useChrom, mean, var);
  ...
  if (paramInFile == "") {
    ...
    InitParams(covCovTransP, covSnvTransP, snvSnvTransP, nStates, nSNVStates,
               log(1 - exp(small)), log(exp(small) / (nStates - 1)), emisP,
               model, maxCov, mean, var, binoP);
    ...
    for (int i = 0; i < 4; i++) {
      ...
      /*
        pChrom = BaumWelchEOnChrom(startP,
                                   ...
                                   expEmisP);
      */
      //	px = PairSumOfLogP(px, pChrom);
      /*
      for (int c=0; c < covBins.size(); c++) {
      ...
      BaumWelchM(startP, covCovTransP, emisP, binoP, model, stateTotCov,
                 stateNCov, expCovCovTransP, expEmisP, priorCovCov,
    ...
    if (paramOutFile != "") {
      WriteParameterFile(paramOutFile, nStates, mean, var, maxState, maxCov,
                         startP, covCovTransP, emisP);
    }
  }

  //
  // Now filter cn=1 and cn=3 intervals.
  //
  for (int c = 0; c < contigNames.size(); c++) {
    int snvStart = 0;
  ...
  WriteVCF(*outPtr, referenceName, sampleName, contigNames, contigLengths,
           copyIntervals);
}
