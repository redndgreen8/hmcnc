    ...
    int clip_thr )
{
    ...
    double clip_pr=0;
    double result;
    int clip = cl_observations[index+1]; //shift clip by -1
    // clip threshold 3
    ...
    int clip_thr)
{
    ...
    for(size_t i=0;i<=nStates;i++)
    {
        v[i][0] =  startP[i] + emissionPr( 1, i , observations,cl_observations,emisP, mean, max_obs,nStates ,clip_thr);
    }
    // Iteration
...
int main(int argc, const char * argv[]) 
{
    ...
    if (argc != 8) 
    {
        std::cerr << "usage: " << argv[0] << " <coverage observation>  <clip observation> <output prefix> <Hmean> <epsi> <clip threshold> <scaler>" << endl;
        return EXIT_FAILURE;
    }
...
//    size_t i=0;
    ...
    //*300
    const double scale = std::stod(argv[7]);
